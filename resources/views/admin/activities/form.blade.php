        ...
        <form action="{{ isset($activity) ? route('activities.update', $activity->id) : route('activities.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                    ...
                    @if(isset($activity) && !empty($activity->images))
                        @php
                            ...
                            $sortedImages = array_merge(
                                array_intersect($activeImages, $activity->images ?? []),
                                array_diff($activity->images ?? [], $activeImages)
                            );
                        @endphp
                        <div class="mb-3">
                                ...
                                <div class="col-auto">
                                    <div class="card" style="width: 120px;">
                                        <div style="height: 90px; overflow: hidden; background: #f8f9fa;">
                                            <img src="{{ Storage::url($img) }}" alt="Activity Image" style="width: 100%; height: 100%; object-fit: cover;">
                                        </div>
                ...
                <div class="col-md-6">
                    ...
                    @if(isset($activity) && !empty($activity->videos))
                        @php
                            $activeVideos = $activity->active_videos ?? [];
                            $sortedVideos = array_merge(
                                array_intersect($activeVideos, $activity->videos ?? []),
                                array_diff($activity->videos ?? [], $activeVideos)
                            );
                        @endphp
                        <div class="mb-3">
...
<form id="delete-media-form" action="{{ isset($activity) ? route('activities.deleteMedia', $activity->id) : '#' }}" method="POST" class="d-none">
    @csrf
    @method('DELETE')
